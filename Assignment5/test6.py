...
def analyze_pincer_movement(piece, row, col, board):
  ...
  while (col + k < 8):
    ...
    # a piece is in the way
      ...
      break
    k = k + 1
  # the next piece is not yours so check for a squish
  if (col + k < 8 and who(board[row][col + k]) != CURRENT_PLAYER):
    # make sure the next next piece is yours so you can squish
    if (col + k < 7 and who(board[row][col + k + 1]) == CURRENT_PLAYER):
      new_board = copy_board(board)
      new_board[row][col + k] = piece
      new_board[row][col] = 0
      new_boards.append(new_board)
  ...
  while (col - k > -1):
    ...
    k = k + 1
  # the next piece is not yours so check for a squish
  if (col - k > -1 and who(board[row][col - k]) != CURRENT_PLAYER):
    # make sure the next next piece is yours so you can squish
    if (col - k > 0 and who(board[row][col - k - 1]) == CURRENT_PLAYER):
      new_board = copy_board(board)
      new_board[row][col - k] = piece
      new_board[row][col] = 0
      new_boards.append(new_board)
  ...
  while (row + k < 8):
    ...
    k = k + 1
  # the next piece is not yours so check for a squish
  if (row + k < 8 and who(board[row + k][col]) != CURRENT_PLAYER):
    # make sure the next next piece is yours so you can squish
    if (row + k < 7 and who(board[row + k + 1][col]) == CURRENT_PLAYER):
      new_board = copy_board(board)
  ...
  '''
  # the next piece is not yours so check for a squish
  if (row - k > -1 and who(board[row - k][col]) != CURRENT_PLAYER):
    # make sure the next next piece is yours so you can squish
    if (row - k > 0 and who(board[row - k - 1][col]) == CURRENT_PLAYER):
      new_board = copy_board(board)
      new_board[row - k + 1][col] = piece
      new_board[row - k][col] = 0
      new_board[row][col] = 0
...
test_state = BC_state(PINCER_TEST_1, WHITE)
print(test_state)
